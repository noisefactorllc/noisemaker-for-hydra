...
- Observed behavior: Implementation commit of 2026-09-26: the artifact now carries its required notices — `public/LICENSE` (verbatim copy of the repository AGPL-3.0 LICENSE) and `public/THIRD-PARTY-NOTICES` (p5.js 1.6.0 LGPL-2.1 embedded notice, Font Awesome Free 5.2.0 license statements, normalize.css 3.0.2, Skeleton V2.0.4, the Tomorrow Night Eighties theme, and the complete bundled npm dependency set (82 packages from the bundler's module graph, with installed versions, per-package copyright notices, and the non-MIT licenses: qs BSD-3-Clause, hyperx BSD-2-Clause, ieee754 BSD-3-Clause, inherits/nanoassert/prettier-bytes ISC, nanoscheduler Apache-2.0)) are emitted into `dist/`, which now tracks 30 files. In the recorded run, the isolated build (git archive of the candidate commit, fresh `npm ci`) reproduced all 30 tracked files byte-exactly (SHA-256 diff exit 0, comparing one isolated build against the tracked tree). A new limit is recorded: ten consecutive isolated builds of the identical tree produced three distinct bundle hashes and three distinct emitted chunk filenames (`13ba4b5c`/index-aecb58b8.js x6, `2f4bce39`/index-df962457.js x3, `9bfff69e`/index-c2ad0356.js x1), differing in esbuild minifier identifier assignment and declaration order; because `dist/index.html` references `index-aecb58b8.js`, a fresh build reproduces the tracked 30-file set only when it emits the tracked chunk name (6 of 10 runs here). The tracked `dist` bytes are bound by hash to this commit; a rebuild is not a byte-exact reproduction of the artifact, and consumers must bind to the committed artifact bytes. All 16 production dependency advisories are assessed for reachability: none is reachable in the distributed default artifact (per the module-graph evidence in `distribution-evidence/bundled-packages.txt`: bundled but dormant behind `VITE_SERVER_URL` or unused code paths, or not bundled at all); no dependency upgrade was made. Served-byte binding: the repository declares no external deployment (`git ls-remote origin` lists only `main`; no `gh-pages`; `https://noisefactorllc.github.io/noisemaker-for-hydra/` returns 404); the only defined deployment — serving `dist` — was verified on loopback: 30/30 files HTTP 200 with served SHA-256 set equal to the tracked `dist` set. The audit-era external demo (source `626f37c21170927e7c24429ca2c8f31cf38905ec`) remains an undocumented external deployment whose URL is not declared in this repository. Installation: the literal README command `npm install` exit 0 (`distribution-evidence/npm-install.log`) alongside the no-symlink/no-lifecycle variant (`npm ci --bin-links=false --ignore-scripts` exit 0). Saved-sketch upgrade: the `?code=` scheme at the previously served revision `626f37c` is source-verified as `btoa(encodeURIComponent(source))` (raw `git show` excerpt retained in `distribution-evidence/saved-sketch-scheme-626f37c.txt`, identical lines in the current tree), and a new unit test restores a fixed `code` parameter recorded under that scheme plus the legacy `show-code=false` parameter; no published npm package or release channel exists, so upgrade means replacing the served `dist`. Acceptance checks: 80 unit tests / 0 fail, 11 browser checks PASS / 0 fail, `vite build` exit 0.
- Evidence: in-repo raw evidence `distribution-evidence/` (`commands.txt`, `exit-codes.txt`, `versions.txt`, `source-hashes.txt`, `dist-hashes.txt`, `unit.log`, `browser.log`, `npm-audit-omit-dev.json`, `bundled-reachability.txt`, `bundled-packages.txt`, `third-party-licenses/`, `npm-install.log`, `build-determinism.txt`, `saved-sketch-scheme-626f37c.txt`, `served-binding.txt`); report section "Distribution and release checks, 2026-09-26 (GAP-003)" in `docs/COMPATIBILITY.md`; historical audit evidence [distribution-reproduction.json](/Users/alex/.codex/automations/noisemaker-port-completion-audit/evidence-audit-20260924-090235/distribution-reproduction.json) and section 3.
- Next action: Resolve the release blockers GAP-001 and GAP-004, then bind the audit-era external demo URL to source (it is not declared in this repository) and, if release qualification is attempted, address the recorded bundle-build nondeterminism.
...
- Required checks: Preserve raw commands, versions, source hashes, full denominators, output, and exit codes for the acceptance checks.
- Last verification: 2026-09-26 (notices added, 30-file isolated reproduction, all 16 advisories assessed, loopback served-byte binding, saved-sketch upgrade test; no closure claimed — GAP-001/GAP-004 blockers and the undeclared external demo URL remain).
...
5. Check distribution notices, dependencies, served provenance, and upgrades for GAP-003 (executed 2026-09-26; remaining: the undeclared audit-era external demo URL, GAP-001/GAP-004 blockers, and bundle rebuild nondeterminism before release qualification).
...
| 2026-09-26, GAP-003 distribution and release checks | this candidate (implementation and record commit) | No closure claimed. GAP-003 acceptance checks executed (notices, artifact hashes, dependency decisions, exact-source served-byte binding, installation, saved-sketch upgrade); GAP-001, GAP-004, GAP-005, GAP-006 unchanged. | 80 unit tests (79 prior + 1 new saved-sketch upgrade test), 11 browser checks, isolated `npm ci` + `vite build` reproducing all 30 tracked dist files byte-exactly, loopback served-byte binding 30/30, `npm audit --omit=dev` 16 advisories assessed. Raw logs in `distribution-evidence/`. | GAP-001 and GAP-004 release blockers open; audit-era external demo URL undeclared; bundle rebuild nondeterminism recorded (3 distinct hashes in 10 builds) and not fixed; advisories assessed, not eliminated. |
