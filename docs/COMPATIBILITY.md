...
| Built distribution | verified (recorded run) | Isolated build matched all 28 tracked distribution files (2026-09-24) and all 30 tracked files after the notices addition (2026-09-26, SHA-256 diff exit 0) — each claim compares one isolated build against the tracked tree. Only dist was served on loopback. Newly recorded limit: ten consecutive isolated builds of the identical tree produced three distinct bundle hashes and chunk filenames (index-aecb58b8.js x6, index-df962457.js x3, index-c2ad0356.js x1), so a fresh build reproduces the tracked 30-file set only when it emits the tracked chunk name; a rebuild is not a byte-exact reproduction of the artifact. |
...
| Artifact notices | verified | `dist/LICENSE` (repository AGPL-3.0 text) and `dist/THIRD-PARTY-NOTICES` (p5.js 1.6.0 LGPL-2.1 embedded notice, Font Awesome Free 5.2.0, normalize.css 3.0.2, Skeleton V2.0.4, Tomorrow Night Eighties theme, bundled MIT npm dependencies with installed versions and per-package copyright notices) are emitted from `public/` into every build (2026-09-26). |
| Dependency advisories | assessed | `npm audit --omit=dev` reports 16 advisories (2 low, 6 moderate, 7 high, 1 critical); reachability analysis shows none reachable in the distributed default artifact (not bundled, not imported, or dormant behind the undefined `VITE_SERVER_URL`). Advisories are assessed, not eliminated (2026-09-26). |
...
Implementation changes in this round: the artifact now carries its required notices — `public/LICENSE` (a verbatim copy of the repository's GNU AGPL-3.0 `LICENSE`, preserving AGPL attribution) and `public/THIRD-PARTY-NOTICES` (attribution for the vendored p5.js 1.6.0 under LGPL-2.1 with its embedded notice, Font Awesome Free 5.2.0 with its per-file license statements, normalize.css 3.0.2, Skeleton V2.0.4, the Tomorrow Night Eighties theme, and the bundled MIT-licensed npm production dependencies with their installed versions). Vite emits both files into `dist/`, so the tracked distribution now contains 30 files (previously 28).

Distribution and installation checks executed (raw commands, output, and exit codes in `distribution-evidence/`):

| Command | Expected | Executed | Pass | Fail | Exit |
|---|---|---|---|---|---|
| `npm ci --bin-links=false --ignore-scripts` (isolated `git archive` tree at `413d97ac`) | install | 1 | 1 | 0 | 0 |
...
| `node --test test/*.test.mjs` | 80 tests | 80 | 80 | 0 | 0 (`distribution-evidence/unit.log`) |
| `CHROME=/usr/bin/chromium PORT=5177 node scripts/test.mjs` | 11 browser checks | 11 | 11 | 0 | 0 (`distribution-evidence/browser.log`) |

Bundle determinism (newly recorded limit): ten consecutive isolated builds of the identical tree produced three distinct bundle hashes — `13ba4b5c…` (6 runs, the tracked bundle), `2f4bce39…` (3 runs), and `9bfff69e…` (1 run) — see `distribution-evidence/build-determinism.txt`. The differences are confined to esbuild minifier identifier assignment and declaration order inside the single emitted chunk (module-level equivalence was not proven beyond this token comparison; the compared variant had 110,901 vs 110,908 identifier tokens with 911 renamed identifiers and 284 short reorder blocks). The tracked `dist/` bytes are therefore bound by hash to this exact commit, but a rebuild is not guaranteed to be byte-identical; consumers must bind to the committed artifact bytes, not to a fresh build. The earlier 2026-09-24 claim that an isolated build reproduced all 28 files byte-exactly remains true for that recorded run; the determinism limit above was not previously recorded and now is, with the raw ten-run output in `distribution-evidence/build-determinism.txt` matching the counts stated here (13ba4b5c x6, 2f4bce39 x3, 9bfff69e x1).

Dependency decisions (16 production advisories, per-advisory reachability in `distribution-evidence/bundled-reachability.txt` and the raw report in `distribution-evidence/npm-audit-omit-dev.json`): none of the vulnerable modules is reachable in the distributed default artifact. `i18next-http-backend` (moderate, path traversal via unsanitised `lng`/`ns`) is imported nowhere (`src/stores/language-store.js:3` is commented out) and absent from the bundle. `form-data` (critical), `min-document`, `minimatch`, `brace-expansion`, `semver`, `formidable`, `ws` (server-side only), and `@babel/runtime` are not bundled — the single `form-data` string hit in the bundle is superagent's MIME-type constant, and form-data's own dependencies (`hexoid`, `combined-stream`) are absent. `@babel/runtime` (via i18next) and `min-document` (via choo→global) are resolved at build time or tree-shaken; `minimatch`, `brace-expansion`, and `semver` reach only through js-beautify's Node-only CLI dependencies and never enter the browser bundle. The remaining bundled-but-dormant chains — `socket.io-client`/`engine.io-client`/`socket.io-parser`/`parseuri` (patch-bay signalling), `superagent`/`qs` (gallery server endpoints), and `shortid` (patch-bay peer IDs) — execute only when `state.serverURL` is set from `VITE_SERVER_URL` (`src/stores/store.js:13`); the distributed build defines no `VITE_SERVER_URL`, so `serverURL` is `null` and these code paths are unreachable. Decision: no dependency is upgraded in this round (upgrade candidates such as shortid→nanoid and socket.io-client 2→4 change public APIs used by the vendored patch-bay code and would alter behavior without rendered qualification); the advisories are recorded as assessed with reachability evidence, and the dormant chains are re-assessable if a server-integrated build is ever produced.

Served-byte binding: the repository declares no external deployment — `git ls-remote origin` lists only `refs/heads/main` at `413d97acde311994fc9fc91b7b7706fbf186cd45` (no `gh-pages`, although `package.json` retains a `publish` script that would subtree-push `dist` to it), and `https://noisefactorllc.github.io/noisemaker-for-hydra/` returns HTTP 404. The only deployment this repository defines is serving the `dist` tree; that binding was verified by serving `dist/` on loopback with `http-server` and fetching every file: 30/30 HTTP 200, and the served SHA-256 set equals the tracked `dist` set exactly (`distribution-evidence/served-binding.txt`). The audit-era external demo (source `626f37c21170927e7c24429ca2c8f31cf38905ec`) remains an undocumented external deployment whose URL is not declared anywhere in this repository; binding its served bytes to source remains impossible from here and stays an open limit.

Saved-sketch upgrade qualification: the `?code=` URL scheme at the previously served revision `626f37c` is source-verified as `btoa(encodeURIComponent(source))` — the verbatim `git show` excerpt is retained in `distribution-evidence/saved-sketch-scheme-626f37c.txt`, and those lines are identical in the current tree (`src/stores/gallery.js:159-164`; `git diff 626f37c..HEAD -- src/stores/gallery.js` touches neither line). A new unit test ('gallery restores a code parameter recorded from the scheme at revision 626f37c', `test/developer-workflow.test.mjs`) restores a fixed `code` parameter recorded under that scheme — committed as a literal so the test restores recorded bytes rather than re-running the current encoder — and also covers the legacy `show-code=false` parameter. The scheme identity is therefore source-inspection evidence plus a round-trip restore through the current decoder, not a test against bytes served by the external demo; the latter stays impossible while that deployment's URL is undeclared. Upgrade path: replace the served `dist` with the new tracked artifact (saved URLs keep working, as verified); no published npm package, release channel, or migration step exists. Installation follows the README (`npm install`, `npm run dev`) with the recorded no-symlink/no-lifecycle variant also passing (`npm ci --bin-links=false --ignore-scripts`, exit 0); removal remains stop-the-server and delete-the-checkout, with no global package or user project touched.
