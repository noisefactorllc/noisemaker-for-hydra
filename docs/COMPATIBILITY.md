...
| Built distribution | verified | Isolated build matches all 28 tracked distribution files (2026-09-24) and all 30 tracked files after the notices addition (2026-09-26, SHA-256 diff exit 0). Only dist was served on loopback. A newly recorded limit: ten consecutive isolated builds produced three distinct bundle hashes (minifier identifier assignment and declaration order only), so a fresh rebuild is not guaranteed byte-identical. |
...
Saved-sketch upgrade qualification: the `?code=` URL scheme is `btoa(encodeURIComponent(source))` and is unchanged since the previously served revision `626f37c` (verified by reading `src/stores/gallery.js` at that commit). A new unit test ('gallery restores a sketch URL saved by the previously served revision 626f37c', `test/developer-workflow.test.mjs`) restores a program from a `code` parameter produced by that exact scheme and also covers the legacy `show-code=false` parameter. Upgrade path: replace the served `dist` with the new tracked artifact (saved URLs keep working, as verified); no published npm package, release channel, or migration step exists. Installation follows the README (`npm install`, `npm run dev`) with the recorded no-symlink/no-lifecycle variant also passing (`npm ci --bin-links=false --ignore-scripts`, exit 0); removal remains stop-the-server and delete-the-checkout, with no global package or user project touched.
